...
RSpec.describe AssetProcessor do
  ...
  let!(:first_asset) { create(:asset) }
  let!(:second_asset) { create(:asset) }
  ...
  it "iterates over all assets" do
    ...
    expect(asset_ids).to contain_exactly(first_asset.id, second_asset.id)
  end
  ...
  context "when scope is set to deleted assets" do
    ...
    it "iterates over all deleted assets" do
      ...
      expect(asset_ids).to contain_exactly(first_asset.id)
    end
  end
