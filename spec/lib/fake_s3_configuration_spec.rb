...
RSpec.describe FakeS3Configuration do
  subject(:config) { described_class.new(env, publishing_platform_config) }

  let(:env) { {} }
  let(:publishing_platform_config) { instance_double(PublishingPlatformConfiguration) }
  ...
  describe "#host" do
    ...
    context "when FAKE_S3_HOST is not set in environment" do
      before do
        allow(publishing_platform_config).to receive(:app_host).and_return(app_host)
      end

      context "when app_host is set" do
        let(:app_host) { "http://example.com" }

        it "returns fake S3 host obtained from Publishing Platform app_host" do
