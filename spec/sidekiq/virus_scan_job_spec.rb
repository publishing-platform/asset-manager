...
RSpec.describe VirusScanJob do
  ...
  it "does not permit multiple jobs to be enqueued for the same asset" do
    SidekiqUniqueJobs.use_config(enabled: true) do
      expect { described_class.perform_in(1.minute, asset.id) }.to enqueue_sidekiq_job(described_class)
      expect { described_class.perform_in(1.minute, asset.id) }.not_to enqueue_sidekiq_job(described_class)
    end
  end
