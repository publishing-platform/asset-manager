...
RSpec.describe Asset, type: :model do
  ...
  describe "validation" do
    subject(:asset) { build(:asset, attributes) }
    ...
    context "when replaced_by_id is not specified" do
      let(:attributes) { { replaced_by_id: nil } }
    ...
    context "when replaced_by_id is specified" do
      let(:attributes) { { replaced_by_id: } }

      context "and replacement asset exists" do
        let(:replacement) { create(:asset) }
        let(:replaced_by_id) { replacement.id }
      ...
      context "and replacement asset does not exist" do
        ...
        it "includes error for replacement not found" do
          asset.valid?
          expect(asset.errors[:replaced_by]).to include("not found")
        end
      end
    ...
    context "when replacements are replaced" do
      let(:first_replacement) { create(:asset, draft: false) }
      let(:second_replacement) { create(:asset, draft: false) }

      before do
        asset.replaced_by = first_replacement
        asset.save!
        first_replacement.replaced_by = second_replacement
        first_replacement.save!
      end

      it "backpropagates replacements" do
        expect(asset.reload.replaced_by_id).to eq(second_replacement.id)
      end

      context "and second replacement is a draft" do
        let(:second_replacement) { create(:asset, draft: true) }

        it "doesn't update the original asset if the second replacement is a draft" do
          expect(asset.reload.replaced_by_id).to eq(first_replacement.id)
        end
      ...
      context "and the URL points to the draft stack" do
        ...
        context "when the asset is published" do
          ...
          it "has the expected error message" do
            asset.valid?
            message = "must be a public publishing-platform.co.uk URL"
            expect(asset.errors[:parent_document_url]).to include(message)
          end
        end
      end
    ...
    context "when content_type is specified" do
      ...
      it "rejects an invalid media type" do
        %w[
          */*
  ...
  describe "#valid_auth_bypass_token?" do
    it "returns true when given an auth_bypass_id which is in the auth_bypass_ids" do
      asset = build(:asset, auth_bypass_ids: %w[my-token])
  ...
  describe "scheduling a virus scan" do
    it "schedules a scan after create" do
      ...
      expect(VirusScanJob).to receive(:perform_async).once

      a.save!
    end

    it "schedules a scan after save if the file is changed" do
      a = create(:clean_asset)
    ...
    it "does not schedule a scan if a redirect url is present" do
      a = create(:asset, redirect_url: "/some-redirect")
  ...
  describe "when an asset is marked as infected" do
    ...
    it "does not schedule saving the asset to cloud storage" do
      expect(SaveToCloudStorageJob).not_to receive(:perform_async)
  ...
  describe "#md5_hexdigest" do
    ...
    before do
      allow(Digest::MD5).to receive(:hexdigest).and_return("md5-from-file")
    end
    ...
    context "when asset is created" do
      ...
      context "when asset is updated with new file" do
        ...
        before do
          allow(Digest::MD5).to receive(:hexdigest).and_return("md5-from-new-file")
