...
RSpec.describe AssetPresenter do
  ...
  describe "#as_json" do
    ...
    before do
      allow(view_context).to receive(:asset_url).and_return(asset_url)
    ...
    context "when the asset has a content type specified" do
      let(:asset) { build(:asset, content_type: "application/pdf") }
    ...
    it "returns hash including public asset URL as file_url" do
      ...
      expect("#{uri.scheme}://#{uri.host}").to eq(PublishingPlatformLocation.asset_root)
    ...
    context "when asset has no replacement" do
      before do
        asset.replaced_by = nil
      end

      it "returns hash without replaced_by_id" do
        expect(json).not_to have_key(:replaced_by_id)
      end
    end
    ...
    context "when asset has replacement" do
      ...
      before do
        asset.replaced_by = replaced_by
      end

      it "returns hash including replacement_id" do
        expect(json).to include(replaced_by_id:)
      end
    end
  end
end
