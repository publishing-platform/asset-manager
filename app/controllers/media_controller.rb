...
class MediaController < ApplicationController
  ...
  def download
    if asset.replaced_by.present? && (!asset.replaced_by.draft? || requested_from_draft_assets_host?)
      set_default_expiry
    ...
    if asset.redirect_url.present?
      redirect_to asset.redirect_url, allow_other_host: true
      return
    end
...
private
  ...
  def requested_from_draft_assets_host?
    request.host == AssetManager.publishing_platform.draft_assets_host
  end

  def requested_from_internal_host?
    request.host == URI.parse(PublishingPlatformLocation.find("asset-manager")).host
  end
  ...
  def redirect_to_draft_assets_host
    redirect_to(
      {
        host: AssetManager.publishing_platform.draft_assets_host,
  ...
  def redirect_to_replacement_for(asset)
    ...
    target_host =
      if asset.replaced_by.draft?
        AssetManager.publishing_platform.draft_assets_host
      else
        AssetManager.publishing_platform.assets_host
      end

    redirect_to "//#{target_host}#{asset.replaced_by.public_url_path}",
  ...
  def set_token_payload
    ...
    @token_payload = if token
                       secret = Rails.application.credentials.jwt_auth_secret
